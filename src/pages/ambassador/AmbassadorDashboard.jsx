...
const taskIcon = (cat) => ({ 'Social Media': <InstagramIcon/>, 'Referral': <UsersIcon/>, 'Learning': <BookIcon/>, 'Content': <PenIcon/>, 'Event': <MicIcon/> }[cat] || <StarIcon/>);
  ...
  return (
    ...
    onMouseLeave={e => { e.currentTarget.style.transform = 'none'; }}>
      ...
      <div>
        ...
        {/* ✅ FIXED: explicit valueColor instead of inherit */}
...
export default function AmbassadorDashboard() {
  const { user } = useAuth();
  const { weeklyActivity, tasks, getLeaderboard } = useData();
  ...
  const toggleTheme = () => {
    setIsDark(prev => {
      ...
      // ✅ fire storage event so AmbassadorNav picks it up immediately
  ...
  // Bar colors — highlight highest bar, show gradient for others
    ...
    <div style={{ minHeight: '100vh', background: t.bg, fontFamily: F, transition: 'background 0.4s' }}>
      ...
      <main style={{ marginLeft: 224, padding: 32, maxWidth: 1100 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px',
              ...
              <div>
                ...
                <p style={{ fontSize: 12, fontWeight: 800, color: t.streakTextBot, fontFamily: F }}>
                  {user?.streak ?? 1}-day streak going!
                </p>
              </div>
            </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 14,
          ...
          <div style={{ flex: 1.5, borderRadius: 24, padding: '24px 28px', display: 'flex',
            ...
            {isDark && [{top:'12%',left:'18%',s:2},{top:'38%',left:'62%',s:1.5},
              ...
              <div key={i} style={{ position:'absolute', top:st.top, left:st.left,
                width:st.s, height:st.s, borderRadius:'50%', background:'white', opacity:0.5, pointerEvents:'none' }} />
            ))}
          ...
          {/* ✅ All StatCards now have explicit valueColor */}
          <StatCard label="Your Level" value={level.name} sub={level.emoji + ' Rank'}
            icon={<CrownIcon />}
          ...
          <StatCard label="Day Streak" value={user?.streak ?? 1} sub="Keep it going!"
            icon={<FireIcon />}
          ...
          <StatCard label="Global Rank" value={`#${userRank}`} sub="Keep climbing!"
            icon={<CrownIcon />}
          ...
          <StatCard label="Tasks Done" value={user?.tasksCompleted ?? 0} sub={`${user?.badges?.length ?? 1} badges`}
            icon={<LightningIcon />}
          ...
          <div style={{ borderRadius: 24, padding: '24px 26px', background: t.card, boxShadow: t.cardShadow,
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                ...
                {/* Total XP this week */}
                <p style={{ fontSize: 10, fontWeight: 700, color: t.subColor, fontFamily: F, display: 'flex', alignItems: 'center', gap: 4 }}>
              ...
              {(weeklyActivity || []).map((d, i) => (
                <div key={i} style={{
                  ...
                  background: d.points === maxPoints
                    ? (isDark ? 'linear-gradient(135deg,#4F46E5,#7C3AED)' : 'linear-gradient(135deg,#4F46E5,#7C3AED)')
                    : t.pillBg,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 14,
          ...
          <div onClick={() => navigate('/ambassador/tasks')}
            ...
            {[{top:'12%',left:'12%',s:2},{top:'32%',left:'72%',s:1.5},
              ...
              <div key={i} style={{ position:'absolute', top:st.top, left:st.left,
                width:st.s, height:st.s, borderRadius:'50%', background:'white', opacity:0.4, pointerEvents:'none' }} />
            ))}
            <div style={{ position: 'relative' }}>
